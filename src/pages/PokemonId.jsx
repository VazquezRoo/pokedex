...
function PokemonId() {
    ...
    const getPersonStatBar = (stat_base) => {
            const percentBarProgres = (stat_base *100)/255
            console.log(percentBarProgres)
    ...
    // console.log(pokedexList[pokemon?.order + 1][0].toUpperCase() + pokedexList[pokemon?.order + 1].substring(1))
    // const numberNextPokedex =  (pokedexList[pokemon?.order + 1]).padStart(4,'0')
    // const numberPreviousPokedex =  (pokedexList[pokemon?.order - 1]).padStart(4,'0')
    ...
    // console.log((stat.base_stat*100)/150)
  ...
  return (
    <section className='grid  min-w-screen bg-[url("/images/fondo2.jpg")] border-2 border-black min-[1280px]:w-[1260px] min-[1600px]:w-[1500px]'>
        ...
        <button className=''><img src="/images/pokedex.png" alt=""  className=' h-[50px] w-[180px]'/></button>
        </Link>  
         ...
         <div className=' h-[100px] flex gap-1 pt-3 justify-between p-2'>
            ...
            {
                pokemon?.id > 1 ? <button to= {`/pokemon/${pokemon?.id - 1}`} className='h-[80px] flex w-[100%] bg-gray-500 items-center justify-between pl-2 pr-3'>
                <i className='bx bx-left-arrow-circle text-[40px] text-white'></i>
                <p className='text-[12px] min-[800px]:pb-6 min-[900px]:text-[30px] min-[400px]:text-[18px] grid min-[800px]:flex min-[800px]:gap-6'>{pokedexList[(pokemon?.id -1).toString()][0].toUpperCase() + pokedexList[(pokemon?.id -1).toString()].substring(1)} <span className=' text-black/40 grid z-30 pt-4 min-[800px]:pt-0'>N.°{numberPreviousPokedex}</span></p>  
                </button>:  
                
                <button  className='h-[60px] flex w-[100%] opacity-0 invisible bg-gray-500 items-center justify-between pl-2 pr-3'>
                <i className='bx bx-left-arrow-circle text-[40px] text-white '></i>
                <p className='text-[12px] min-[900px]:text-[30px] min-[400px]:text-[18px]   grid min-[800px]:flex min-[800px]:gap-6'></p>
                </button>

            }
            <div className={`trapecio__dos ${pokemon?.id > 1 ? 'opacity-100' : 'opacity-0 invisible'}`}></div>
            </Link>
            ...
            <p className='text-[12px] min-[900px]:text-[30px] min-[400px]:text-[18px] grid min-[800px]:flex min-[800px]:gap-6 min-[800px]:pb-6'>{pokedexList[(pokemon?.id +1).toString()]} <span className='text-black/40 grid pt-3 z-30 min-[800px]:pt-[0] '>N.°{numberNextPokedex}</span></p>
        ...
        <h3 className='text-[25px]'>{pokemon?.name[0].toUpperCase() + pokemon?.name.substring(1)}  <span className='text-black/50'>N.°{numberPokedex}</span></h3>
        </div>
        ...
        <div className={`${backgroundByType[pokemon?.types[0].type.name]} mt-[150px] w-[260px] h-[100px] relative min-[1100px]:w-[90%]`}>
            ...
            <button onClick={changeShiny} className='absolute right-0 -mr-[0px] -mt-[150px] flex items-center gap-1'><i class='bx bxs-paint text-[30px]'></i></button>
        </div>
        ...
        <h3 className='text-[35px]'><span className='bg-white p-3'>{pokemon?.name[0].toUpperCase() + pokemon?.name.substring(1)} </span> <span className='text-black/50 bg-white p-3'>N.°{numberPokedex}</span></h3>
        </div>
       ...
       <p >No info</p>
        ...
        {/* Stats pokemon */}
            ...
            {/* <div className=' w-[260px] min-[750px]:w-[500px] bg-black/50 min-[1000px]:w-[260px]'>
           ...
           <div className='flex w-[50%] min-[1000px]:w-[60%]  '>
            <div className='w-[100%] min-[1000px]:w-[60%] text-center  border-r-2 border-black h-[100%] ' >Movent</div>
            <div className=' opacity-0 invisible absolute min-[1000px]:opacity-100  min-[1000px]:relative min-[1000px]:visible min-[1000px]:w-[20%] text-center  border-r-2 border-black  ' >Accuracy</div>
            <div className=' opacity-0 invisible absolute min-[1000px]:opacity-100  min-[1000px]:relative min-[1000px]:visible  min-[1000px]:w-[20%] text-center  border-r-2 border-black ' >Power</div>
            </div>
